#include "Arduino.h"
// NOTE: Avoid Serial.print() its results can cause random keystrokes
...
// Checks switches status if it was just turned on send its assigned
// character and return its new status
...
void loop() {
	...
	for (int i = 0; i < 10; ++i) {
		Serial.flush();
		S_toggle_VALS[i] = check_Switch(S_toggle_pins[i], S_toggle_VALS[i],
				S_toggle_command[i]);
		Serial.flush();
	}
	//
